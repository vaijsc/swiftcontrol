...
from dataloader.utils import get_dataset
from dataloader.utils import encode_prompt
from swiftbrush.validator import SBControlValidator
...
def main(args):
    ...
    teacher_lora_controlnet = LoRAControlNetModel.from_pretrained(
        args.teacher_controlnet_model_name_or_path, torch_dtype=weight_dtype, use_safetensors=False)
    ...
    config = LoraConfig(
        ...
        target_modules=["to_q", "to_v", "controlnet_cond_embedding.blocks.5", "controlnet_cond_embedding.conv_out"],
    ...
    # student load
    if args.random_init:
        raise ValueError("not supported ControlNet")
        unet = UNet2DConditionModel.from_config(
            student_path, subfolder=student_subfolder
        )
    else:
        student_controlnet = ControlNetModel.from_pretrained(args.pretrained_student_controlnet_path, torch_dtype=weight_dtype, use_safetensors=False)
    ...
    if args.use_ema:
        if args.random_init:
            ...
            ema_controlnet = ControlNetModel.from_pretrained(args.pretrained_student_controlnet_path, torch_dtype=weight_dtype, use_safetensors=False)
    ...
    def preprocess_train(examples):
        ...
        cond_images = [cond_image for cond_image in examples['cond_image']]
        examples["pixel_values"] = torch.stack([image_transforms(image) for image in images]).squeeze()
        examples["conditioning_pixel_values"] = torch.stack([conditioning_image_transforms(cond_image) for cond_image in cond_images]).squeeze()
        return examples
