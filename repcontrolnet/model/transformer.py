...
class RepAttention(Attention):
    def __init__(self, *args, **kwargs):
        ...
        if not self.pre_only:
            self.to_out_rep = copy.deepcopy(self.to_out)
...
class RepAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # linear proj
        hidden_states_rep = attn.to_out_rep[0](hidden_states)
        hidden_states = attn.to_out[0](hidden_states)
        hidden_states = hidden_states + hidden_states_rep
        # dropout
...
class RepFeedForward(nn.Module):
    ...
    def __init__(
        ...
        self.net_rep = nn.ModuleList([])
        # project in
        self.net_rep.append(act_fn)
        # project dropout
        self.net_rep.append(nn.Dropout(dropout))
        # project out
        self.net_rep.append(nn.Linear(inner_dim, dim_out, bias=bias))
        # FF as used in Vision Transformer, MLP-Mixer, etc. have a final dropout
        if final_dropout:
            self.net_rep.append(nn.Dropout(dropout))
    ...
    def forward(self, hidden_states: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        if len(args) > 0 or kwargs.get("scale", None) is not None:
            deprecation_message = "The `scale` argument is deprecated and will be ignored. Please remove it, as passing it will raise an error in the future. `scale` should directly be passed while calling the underlying pipeline component i.e., via `cross_attention_kwargs`."
            deprecate("scale", "1.0.0", deprecation_message)
        for module, module_rep in zip(self.net, self.net_rep):
            hidden_states_rep = module_rep(hidden_states)
...
class RepTransformer2DModel(Transformer2DModel):
    ...
    def _init_continuous_input(self, norm_type):
        ...
        self.proj_in_rep = copy.deepcopy(self.proj_in)
        ...
        self.proj_out_rep = copy.deepcopy(self.proj_out)
    ...
    def _operate_on_continuous_inputs(self, hidden_states):
        ...
        if not self.use_linear_projection:
            hidden_states_rep = self.proj_in_rep(hidden_states)
    ...
    def _get_output_for_continuous_inputs(self, hidden_states, residual, batch_size, height, width, inner_dim):
        if not self.use_linear_projection:
            ...
            hidden_states_rep = self.proj_out_rep(hidden_states)
