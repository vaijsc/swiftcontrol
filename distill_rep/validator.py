...
#bug
class SBValidator():
    def __init__(self, device, noise_scheduler, vae, unet, tokenizer, text_encoder, generator=None):
        self.device = device
...
class SBControlValidator():
    def __init__(self, device, noise_scheduler, vae, 
                 unet, tokenizer, text_encoder, generator=None):
        self.device = device
        self.noise_scheduler = noise_scheduler
        self.vae = vae
        self.unet = unet
    ...
    def encode_prompt(self, prompt, batch_size=1):
        ...
        return encoder_hidden_state
    @torch.no_grad
    def generate_latent(self, noise, encoder_hidden_state, controlnet, controlnet_image):
    ...
    def decode_image(self, latent):
        ...
        return image
    @torch.no_grad
    def generate_image(self, controlnet, prompt=None, noise=None, return_latent=False):
        ...
        latent = self.generate_latent(noise, encode_hidden_state, controlnet, self.cond_images)
        image = self.decode_image(latent)
        if return_latent:
            return latent, image
        return image
    ...
    @torch.no_grad
    def __call__(self, controlnet):
        gen_image = self.generate_image(controlnet.to(device=self.device, dtype=self.weight_dtype))
        return gen_image
...
if __name__ == "__main__":
    ...
    full_path_save = osp.join(path_save, "test_sbcontrol_validator.jpg")
    save_image(gen_image, full_path_save)
    print(f"saved image to {full_path_save}")