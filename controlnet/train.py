    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                encoder_hidden_states = batch["input_ids"].to(dtype=weight_dtype)
            ...
            if accelerator.sync_gradients:
                if args.use_ema:
                    if args.offload_ema:
                        ema_controlnet.to(device="cuda", non_blocking=True)
                    ema_controlnet.step(controlnet.parameters())
                    if args.offload_ema:
                        ema_controlnet.to(device="cpu", non_blocking=True)
